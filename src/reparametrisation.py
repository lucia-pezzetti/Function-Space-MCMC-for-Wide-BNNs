...
def _get_ident_energy_fn(f, noise_scale, parallel):
	...
	def neg_log_likelihood_fn(phi, x, y):
		theta = phi

		neg_log_likelihood = 0.5 * jnp.sum((y - f(theta, x)) ** 2 - lvar) / lvar

		if parallel:
			neg_log_likelihood = jax.lax.psum(neg_log_likelihood, axis_name='i')

		return neg_log_likelihood

	return energy_fn, neg_log_likelihood_fn
...
def _get_nngp_energy_fn(
	...
	*reparametrisation*. Uses the effective implementation via Cholesky. Assumes
	...
	def neg_log_likelihood_fn(phi, x, y):
		...
		neg_log_likelihood = 0.5 * jnp.sum((y - pred) ** 2 - lvar) / lvar
		# logdet = d_out * jnp.sum(0.5 * jnp.log(kernel_reg) - jnp.log(jnp.diag(cho)))
