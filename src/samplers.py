...
def pCN(energy_fn, logp_diff_fn, stepsize):
	...
	def sample_fn(key, state, x, y):
		...
		# get a new proposal
		key, pkey = random.split(key)
		phi_new = jnp.sqrt(1.0 - stepsize**2) * phi
		phi_new += stepsize * random.normal(pkey, phi.shape)
		energy_new, (theta_new, logdet_new, stats) = energy_fn(phi_new, x, y)
...
def pCNL(energy_fn, neg_log_likelihood_fn, logp_diff_fn, delta):
	"""preconditioned Crank-Nicholson with Langevin.
	
	Args:
		energy_fn:
			A function computing the neg. log posterior density (up to a constant).
			See `get_energy_fn` in `reparametriation.py` for details.
		neg_log_likelihood_fn:
			A function computing the negative log likelihood density (up to a constant).
			See `get_energy_fn` in `reparametriation.py` for details.
		logp_diff_fn:
			A function which computes the difference between log density values at
			two different values of `theta` (i.e., the parameter itself for the
			...
			See `get_logp_diff_fn` in `reparametriation.py` for details.
		delta:
			Step size for the Langevin proposal.
	...
	neg_log_likelihood_vg = jax.value_and_grad(neg_log_likelihood_fn, has_aux=False)
	...
	def sample_fn(key, state, x, y):
		...
		# get a new proposal
		key, pkey = random.split(key)
		neg_log_likelihood, g = neg_log_likelihood_vg(phi, x, y)

		phi_new = (2.0 - delta) * phi - 2.0 * delta * g
		phi_new += jnp.sqrt(8*delta) * random.normal(pkey, phi.shape)
		phi_new *= 1.0 / (2.0 + delta)

		energy_new, (theta_new, logdet_new, stats) = energy_fn(phi_new, x, y)
		neg_log_likelihood_new, g_new = neg_log_likelihood_vg(phi_new, x, y)

		# accept is 1 if proposal accepted, 0 otherwise
		# log_p_acc = logp_diff_fn(theta, theta_new, x, y) + logdet_new - logdet
		# log_p_acc += 0.5 * jnp.sum(phi_new**2 - phi**2)
		# log_p_acc += 0.25 * jnp.sum(g**2 - g_new**2)
		# log_p_acc += 0.25 * (2 + delta) * (jnp.dot(phi_new, g) - jnp.dot(phi, g_new))
		# log_p_acc += 0.25 * (2 - delta) * (jnp.dot(phi_new, g_new) - jnp.dot(phi, g))
		log_p_acc = neg_log_likelihood - neg_log_likelihood_new
		log_p_acc -= 0.5 * jnp.dot(phi - phi_new, g + g_new)
		log_p_acc += 0.25 * delta * jnp.dot(phi + phi_new, g - g_new)
		log_p_acc += 0.25 * delta * (jnp.sum(g**2 - g_new**2))
		p_acc = jnp.minimum(1, jnp.exp(log_p_acc))
