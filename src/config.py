...
def get_config():
  ...
  C.stepsize = 0.1  # sampler stepsize (applies to `hmc`, `pCN` and `gd`)
  C.delta = 0.01  # sampler stepsize (applies to `pCNL`)
  ...
  C.step_count = 10000000 # number of steps the sampler is to take
  C.burn_in = 20000  # number of burn-in steps before `step_count` starts
  C.thin = 25  # thinning factor (does not increase `step_count`!)
  ...
  C.n_units = 128  # number of hidden embeddings dimensions (see `models.py`)
  ...
  C.reparam_type = 'nngp'  # 'identity', 'nngp'
