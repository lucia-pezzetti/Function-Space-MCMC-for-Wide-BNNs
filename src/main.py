...
def _init_mcmc(init_fn, apply_fn, embed_fn, unflatten, C):
	...
	energy_fn, neg_log_likelihood_fn = reparam.get_energy_fn(
	...
	if sampler == 'hmc':
		...
		_sampler_init, _sampler_step = samplers.gauss_mh(  # pylint: disable=invalid-name
				energy_fn=energy_fn, mcmc_beta=mcmc_beta, logp_diff_fn=logp_diff_fn)
	elif sampler in ('pcn', 'preconditioned_crank_nicolson'):
		_sampler_init, _sampler_step = samplers.pCN(  # pylint: disable=invalid-name
				energy_fn=energy_fn, stepsize=stepsize, logp_diff_fn=logp_diff_fn)
	elif sampler in ('pcnl', 'preconditioned_crank_nicolson_langevin'):
		_sampler_init, _sampler_step = samplers.pCNL(   # pylint: disable=invalid-name
				energy_fn=energy_fn, neg_log_likelihood_fn=neg_log_likelihood_fn, delta=delta, logp_diff_fn=logp_diff_fn)
	else:
